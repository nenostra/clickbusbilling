...
function RfcForm({ handleSubmit }) {
  return (
    <form
      className="RfcForm"
      onSubmit={ handleSubmit(submitAndDispatch)}
    >
      ...
      <div>
        <Field
          ...
          type="text"
          formClassName="RfcForm-input"
          label="rfc"
          validate={[required, rfcValidate]}
        />
      </div>
      <button
        className="RfcForm-submit"
        type="submit"
      >
        <span className="RfcForm-submit-text">
          Entrar
        </span>
      </button>
    </form>
